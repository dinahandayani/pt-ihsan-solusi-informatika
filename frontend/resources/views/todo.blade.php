...
@section('content')
    <div class="rounded-lg">
        ...
        <form action="{{ route('tasks.store') }}" method="POST" class="text-center">
            ...
            <button type="submit" class="bg-blue-400 text-black px-4 py-1 rounded text-sm cursor-pointer hover:bg-blue-300 transition">
                Add Task
            </button>
        </form>
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex flex-col">
                        <div class="flex items-center">
                            ...
                            <a href="{{ route('tasks.edit', $task_o['id']) }}" class="text-gray-600 hover:text-gray-900 cursor-pointer" data-tooltip="Edit">
        ...
        @else
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex flex-col">
                        <div class="flex items-center">
                            ...
                            <a href="{{ route('tasks.edit', $task_c['id']) }}" class="text-gray-600 hover:text-gray-900 cursor-pointer" data-tooltip="Edit">
                                <i class="fas fa-pencil-alt text-sm"></i>
                            </a>
                        </div>
                        <div class="text-sm text-gray-600 mt-1 text-[12px]">
                    ...
                    <div class="flex items-center">
                        <!-- Tombol Hapus -->
                        <a href="#" onclick="event.preventDefault(); document.getElementById('delete-task-{{ $task_c['id'] }}').submit();"
                           class="border border-gray-400 rounded-full w-6 h-6 items-center flex justify-center mr-2 hover:bg-red-200 cursor-pointer" 
                           data-tooltip="Delete">
                           <i class="fas fa-times text-[12px]"></i>
                        </a>
                        <!-- Tombol Completed -->
                        <a href="#" onclick="event.preventDefault(); document.getElementById('complete-task-{{ $task_c['id'] }}').submit();"
                            class="bg-white border border-gray-400 rounded-full w-6 h-6 items-center flex justify-center mr-2 hover:bg-green-200 cursor-pointer" 
                            data-tooltip="Completed">
                            <!-- Ganti dengan ikon sesuai keinginan -->
                        </a>
            ...
            <!-- Form untuk hapus task -->
            <form id="delete-task-{{ $task_c['id'] }}" action="{{ route('tasks.destroy', $task_c['id']) }}" method="POST" class="hidden">
                @csrf
                @method('DELETE')
            </form>
            <!-- Form untuk tanda task selesai -->
            <form id="complete-task-{{ $task_c['id'] }}" action="{{ route('tasks.signtocompleted', $task_c['id']) }}" method="POST" class="hidden">
                @csrf
                @method('PUT')
            </form>
            @endforeach
